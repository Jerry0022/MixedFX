...
public class Layouter {
    ...
    /**
     * @param pane   Superpane which will be used for load() tasks while saving and as root for all children which might can be layouted!
     * @param config
     */
     ...
     *
     * @param paneToShowSaving On this pane load() is called if a new layout background is saved! May be null, then the layout is saved in the background asynchronously without visuals.
     * @param root             Itself and all of its children are scanned if they are layoutables! Overwrites the settings of LayoutManager! Must not be null!
     * @param layoutManager    The LayoutManager to use. Must not be null!
     * @param config           The config for this layout! If null a default config is loaded.
     */
    public static void setLayoutable(final SuperPane paneToShowSaving, final Parent root, final LayoutManager layoutManager, EasyModifierConfig config) {
        ...
        final EasyModifierHandler handler = (parent, doIt) ->
        {
            ...
            final EventHandler<MouseEvent> event = event2 ->
            {
                ...
                final Button button = new Button("Bild ausw�hlen!");
                ...
                button.setOnAction(event1 ->
                {
                    ...
                    imageChooser.setTitle("Bild ausw�hlen!");
