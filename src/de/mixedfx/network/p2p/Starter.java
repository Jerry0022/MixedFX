...
public class Starter
{
	...
	List<Listener> apps = new ArrayList<>();
	...
	public Starter(int bindport, InetSocketAddress bootaddress, int numNodes, Environment env) throws Exception
	{
		...
		for (int curNode = 0; curNode < numNodes; curNode++)
		{
			...
			Listener app = new Listener(node);
		...
		for (int i = 0; i < 10; i++)
		{
			...
			// for each app
			Iterator<Listener> appIterator = apps.iterator();
			while (appIterator.hasNext())
			{
				Listener app = appIterator.next();
	...
	public static void main(String[] args) throws Exception
	{
		...
		try
		{
			...
			InetAddress bootaddr = InetAddress.getByName("192.168.178.55");
			int bootport = 7001;
			...
			// launch our node!
			Starter dt = new Starter(bindport, bootaddress, numNodes, env);
		} catch (Exception e)
		{
			e.printStackTrace();
